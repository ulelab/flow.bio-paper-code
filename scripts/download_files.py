...
''')
    ...
    # Create wrapper script that submits batches sequentially
    # Waits for each batch to complete before submitting the next,
    # to avoid hitting AssocMaxSubmitJobLimit
    submit_script = os.path.join(slurm_dir, "submit_all_batches.sh")
    with open(submit_script, "w") as f:
        f.write(f'''#!/bin/bash
# Submit {n_batches} batch(es) of download jobs ({n_jobs} total, max {max_array} per batch)
# Waits for each batch to finish before submitting the next to stay within job limits.
...
''')
        for i, bs in enumerate(batch_scripts):
            ...
            f.write(f'''
echo "=== Batch {i + 1}/{n_batches} ==="
JOB_ID=$(sbatch --parsable {bs_name})
if [ -z "$JOB_ID" ]; then
    echo "ERROR: Failed to submit {bs_name}. Retrying in 60s..."
    sleep 60
    JOB_ID=$(sbatch --parsable {bs_name})
    if [ -z "$JOB_ID" ]; then
        echo "ERROR: Still failed. Submit remaining batches manually."
        exit 1
    fi
fi
